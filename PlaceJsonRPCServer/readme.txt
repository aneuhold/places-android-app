...
the following methods, which are implemented by the server:
   ...
   public String[] getNames();          //get a string array of titles known to the server
...
(1) You will need to provide execution permission to the scripts (jar strips it) execute:
chmod u+x *.sh
